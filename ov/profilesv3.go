...
func (c *OVClient) CreateProfileFromTemplateWithI3S(name string, template ServerProfile, blade ServerHardware, osDeploymentPlan i3s.DeploymentPlan, deploymentSettings map[string]string ) error {
	...
	serverDeploymentAttributes := make([]OSCustomAttribute, len(osDeploymentPlan.CustomAttributes))
    for i := 0; i < len(osDeploymentPlan.CustomAttributes); i++{
    	customAttribute := &osDeploymentPlan.CustomAttributes[i]
	...
	return fmt.Errorf("%+v", new_template)
