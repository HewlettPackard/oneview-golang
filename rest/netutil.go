...
func (c *Client) newHTTPClient() *http.Client {
	...
	log.Debugf("TEMP TLS CONFIG => SSLVerify=%t InsecureSkipVerify=%t MinVersion=0x%04x",
...
func (c *Client) RestAPICall(method Method, path string, options interface{}, query ...map[string]interface{}) ([]byte, error) {
	...
	// TEMP DEBUG: remove after Jenkins validation is complete.
	if resp.TLS != nil {
		log.Debugf("TEMP TLS HANDSHAKE => complete=%t version=0x%04x cipher=%s serverName=%s verifiedChains=%d",
		...
		log.Debugf("TEMP TLS HANDSHAKE => no TLS state on response")
	}
